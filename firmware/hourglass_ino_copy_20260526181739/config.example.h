// Configuration template for AaltoProtoFlow
// Copy this file to config.h and fill in your credentials
// NOTE: config.h is in .gitignore and will not be committed to version control

#ifndef CONFIG_H
#define CONFIG_H

// WiFi Configuration
// Replace with your WiFi network name and password
const char* WIFI_SSID = "Koti_5AyB";
const char* WIFI_PASSWORD = "RUPKPYR9GSKR2";

// WebSocket Configuration
// Replace with the server URL provided by your instructor
const char* WEBSOCKET_URL = "wss://broker-production-f229.up.railway.app/ws/device";

// SSL Certificate (Let's Encrypt Root CA - update if needed)
const char* ROOT_CA_CERT = R"(
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
)";

#endif // CONFIG_H